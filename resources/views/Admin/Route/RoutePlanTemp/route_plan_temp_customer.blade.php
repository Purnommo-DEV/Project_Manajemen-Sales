@extends('Layout.master', ['title' => 'Data Rute Perjalanan Temporary Customer'])
@section('nav')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="javascript:;">Rute Perjalanan
                    Temporary/</a></li>
            <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="javascript:;">Rute Perjalanan Temporary
                    Customer</a></li>
        </ol>
        <h6 class="font-weight-bolder mb-0">Rute Perjalanan Temporary Customer</h6>
    </nav>
@endsection
@section('konten')
                ...
                <div class="col pb-10">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="modal fade text-left" id="modalTambahRutePlanTemp" data-bs-backdrop="static"
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('admin.TambahRutePlanTempCustomer') }}"
                                            id="formTambahRutePlanTempCustomer" method="POST">
                                            @csrf
                                            <input type="hidden" name="id_route_plan_temp"
                                                value="{{ $rute_plan_temp->id }}" hidden>
                            ...
                            <table class="table table-striped" id="table-data-rute-temp">
                                <thead>
                                    <tr>
                                        <th>No.</th>
                                        <th>Nama Customer</th>
...
@section('script')
    <script>
        let rute_plan_temp = @json($rute_plan_temp);
        let daftar_data_rute = [];
        const table_data_rute = $('#table-data-rute-temp').DataTable({
            "destroy": true,
            "pageLength": 10,
            "lengthMenu": [
                [10, 25, 50, 100, -1],
            ...
            "sScrollXInner": "100%",
            ajax: {
                url: "/admin/data-rute-plan-temp-customer/" + rute_plan_temp.id,
                type: "POST",
                headers: {
        ...
        $('#formTambahRutePlanTempCustomer').on('submit', function(e) {
            e.preventDefault();
            $.ajax({
                ...
                success: function(data) {
                    if (data.status == 0) {
                        ...
                        $("#formTambahRutePlanTempCustomer")[0].reset();
        ...
        $(document).on('click', '.hapus_data_rute_plan_temp', function(event) {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "/admin/hapus-rute-plan-temp-customer/" + id,
                        dataType: 'json',
