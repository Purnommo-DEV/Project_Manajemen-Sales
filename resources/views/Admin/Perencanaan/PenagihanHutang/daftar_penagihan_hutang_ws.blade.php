                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="form-group">
                                                    <select name="tempo" class="form-control">
                                                        <option value="" disabled selected>Pilih Tempo Hari
                                                        </option>
                                                    ...
                                                    <div class="input-group has-validation">
                                                        <label class="text-danger error-text tempo_error"></label>
                                                    </div>
                                                </div>
            ...
            <div class="modal-content">
                ...
                <form id="formEditKunjungiCustomer" action="{{ route('admin.UbahDataPenagihanHutangWholesale') }}"
                    ...
                    <div class="modal-body">
                        ...
                        <label>Temp</label>
                        <div class="form-group">
                            <select name="tempo" class="form-control" id="pilih-tempo"></select>
                        </div>
                        <label>Tanggal Penjualan</label>
                        <div class="form-group">
                            <input type="datetime-local" class="form-control" name="penjualan">
...
@section('script')
    <script>
        ...
        const table_data_kunjungi_customer = $('#table-data-penagihan-hutang').DataTable({
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        daftar_data_kunjungi_customer[row.id] = row;
                        // return moment(row.jatuh_tempo).subtract(1, "days").format(
                        //     'dddd, DD-MM-YYYY, HH:mm:ss');

                        // return moment(row.jatuh_tempo).format('dddd, DD-MM-YYYY, HH:mm:ss');
                        var b = moment(row.penjualan);
                        var a = moment(row.jatuh_tempo);
                        var tempo = a.diff(b, 'days');
                        ...
                        if (tempo == 7) {
                            ...
                            var tempo = a.diff(b, 'days');
                            if (tempo == 6) {
                                return `<lable class="text-success">${moment(row.jatuh_tempo).format('dddd, DD-MM-YYYY, HH:mm:ss')}</label>`;
                            } else if (tempo == 3) {
                                return `<lable class="text-warning">${moment(row.jatuh_tempo).format('dddd, DD-MM-YYYY, HH:mm:ss')}</label>`;
                            } else if (tempo == 1) {
                                return `<lable class="text-danger">${moment(row.jatuh_tempo).format('dddd, DD-MM-YYYY, HH:mm:ss')}</label>`;
                            } else {
                                return `<label class="text-danger">Lewat ${b.diff(a, 'days')} Hari</label>`;
                            }
                        } else if (tempo == 14) {
                            var b = moment();
                            var a = moment(row.jatuh_tempo);
        ...
        $(document).on('click', '.edit_kunjungi_customer', function(event) {
            ...
            $("#formEditKunjungiCustomer [name='tempo']").append(
                $(
                    `
                <option value='' disabled selected>-- Pilih Tempo Hari --</option>
                <option value='7' ${'7' === data_kunjungi_customer.tempo ? 'selected' : ''}>7 Hari</option>
                <option value='14' ${'14' === data_kunjungi_customer.tempo ? 'selected' : ''}>14 Hari</option>
                <option value='21' ${'21' === data_kunjungi_customer.tempo ? 'selected' : ''}>21 Hari</option>
                `
                ))
